...
@Model.register("seq2seq_copy_srl")
class SRLCopyNetSeq2Seq(Model):
    ...
    binary_pred_feature_dim : ``int``
        The size of the embeddings for the predicate indicator feature.
        If set to 0 then the model omits these embedings.
    language_flag_dim : ``int``
        The size of the embeddings for the language embedding.
        If set to 0 then the model omits these embeddings.
    number_of_languages : ``int``
        Number of languages that the model will process (on both Encoder and Decoder sides)
        e.g. For an MT model English to German set number_of_languages = 2.
    ...
    def forward(self,  # type: ignore
        ...
        verb_indicator: torch.LongTensor,
            Tensor containing a 1 where the token is a predicate and 0's otherwise.
        language_enc_indicator: torch.LongTensor,
            Tensor representing the source language
        language_dec_indicator: torch.LongTensor,
            Tensor representing the target language
